...
pub fn main(api) {
   ...
   // ACK
    ...
    // api.on_basic_block(Some(symbolizer::resolve("ull_conn_rx")?), |_| log::info!("===========ull_conn_rx==========="));
  ...
  fn apply_patches() {
    ...
    common::register_basic_block(symbolizer::resolve("ull_conn_rx")?);
  }
  ...
  fn hook_link_layer(api, cfg){
      ...
      // TX
      api.on_basic_block(Some(symbolizer::resolve("radio_pkt_tx_set")?), 
                |_| handle_link_layer_packet(cfg, register::read("r0")?, 1));
      // RX
      api.on_basic_block(Some(symbolizer::resolve("radio_pkt_rx_set")?), 
  ...
  fn handle_link_layer_packet(cfg, pkt_buf_addr, direction) {
    ...
    if (direction == 1) {
      ...
      if cfg.log_packet {
        let pkt_summary = common::parse_packet("ble", pkt_hex, direction, cfg.data_connection, cfg.log_details);
      ...
      match cfg.sequence {
        ...
        // 1 => rx_pdu = "8522a942f80f51c300000000000c7083329a9c9a17020100100000006400ffffffff1f055c7830302939",
        1 => rx_pdu = "8522a942f80f51c300000000000c7083329a9c9a17020100180000002a00ffffffff1f055c7830302939",
        2 => {
        ...
        // 4 => rx_pdu = "03090014fb004808fb004808", // LL_LENGTH_REQ
        // 5 => rx_pdu = "0306000c0800000000", // LL_VERSION_IND
        // -------------------
        // 3 => rx_pdu = "030900082100000000000000", // LL_FEATURE_REQ
        // 6 => rx_pdu = "03090014fb004808fb004808", // LL_LENGTH_REQ
        // 9 => rx_pdu = "0306000c0800000000", // LL_VERSION_IND
        // 12 => rx_pdu = "0207000300040002f700", // ATT_Exchange_MTU_Request(mtu=247)
        // ------------------
        3 => rx_pdu = "030900082100000000000000", // LL_FEATURE_REQ
        5 => rx_pdu = "03090014fb004808fb004808", // LL_LENGTH_REQ
        7 => rx_pdu = "0306000c0800000000", // LL_VERSION_IND
        9 => rx_pdu = "0207000300040002f700", // ATT_Exchange_MTU_Request(mtu=247)
        _ => rx_pdu = "0900",
      }
      ...
      if cfg.log_packet {
        let pkt_summary = common::parse_packet("ble", rx_pdu, direction, cfg.data_connection, cfg.log_details);
