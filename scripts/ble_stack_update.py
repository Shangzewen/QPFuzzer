...
def parse_ble_packet(
    ...
    if data_channel:
        ...
        # print(f"[raw_pkt: data_channel] {hexlify(raw_pkt)}")
...
def generate_reply_data(pkt, flag2):
    ...
    if "LL_FEATURE_RSP" in ble_packet:
        ...
        if flag2 == "0":
            # parse_ble_packet(raw_packet_bytes,0)
            print("==========First Empty_odu Received, Sent LL_FEATURE_REQ==========")
            ...
            # Sent the feature req, update the flag2 only send empty pdu from now on
            flag2 = "1"
        else:
    ...
    return hexlify(bytes(rpl_pkt)), 8, pkt_summary,flag2
