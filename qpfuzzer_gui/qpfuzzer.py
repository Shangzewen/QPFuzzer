    ...
    for line in process.stdout:
        if 'RX' in line or "TX" in line:
            if 'PKT' not in line:
                ...
                output_markdown1.set_content(output_markdown1.content + '\n' +clean_line + '\n')  # Append new output
            else:
                continue
        else:
            continue
...
with ui.row().classes('w-full justify-center items-center'):
    ...
    with ui.dropdown_button().props('outline round').classes('shadow-lg items-center justify-center'):
    ...
    with ui.row().classes('gap-4 m-11'):
        with ui.column().classes("items-center justify-center"):
            ...
            with ui.card().classes('w-[45vw] h-[70vh] flex items-center justify-center'):
                output_markdown1 = ui.markdown("").classes('w-full h-full overflow-y-auto flex-grow')  # Using markdown for output
