...
pub fn main(api) {
    ...
    api.on_instruction(Some(0x00077e3a), |_| register::write("r6",0x00)?);
    api.on_instruction(Some(0x00077e3a), |_| register::read("r6")?);


    // BLE Setup
    // api.on_instrmainuction(Some(symbolizer::resolve("bt_enable")?), |_| log::info!("===========bt_enable=========="));
  ...
  fn apply_patches() {
    ...
    // common::patch_function("nrf_gpiote_in_event_get", arm::RETURN_0);
